...
impl State {
    fn new(count: usize) -> Self {
        Self { selected: vec![false; count], collapsed: HashSet::new(), cursor: 0, h_scroll: 0 }
    }
...
fn run<B: Backend>(terminal: &mut Terminal<B>, actions: &[Action]) -> Result<Vec<usize>, Error> {
    ...
    loop {
        ...
        match read_key()? {
            Key::Up => {
                state.cursor = if state.cursor > 0 { state.cursor - 1 } else { visible.len().saturating_sub(1) };
            }
            Key::Down => {
                state.cursor = if state.cursor + 1 < visible.len() { state.cursor + 1 } else { 0 };
            }
            Key::Toggle => {
                match &visible[state.cursor] {
                    VisibleRow::FileHeader { file } => {
                        let all_on = actions.iter().zip(state.selected.iter())
                            .filter(|(a, _)| a.file == *file)
                            .all(|(_, s)| *s);
                        for (a, s) in actions.iter().zip(state.selected.iter_mut()) {
                            if a.file == *file { *s = !all_on; }
                        }
                    }
            ...
            Key::Invert => { for s in &mut state.selected { *s = !*s; } }
...
enum Key {
    Up, Down, Toggle, ToggleAll, ToggleFile, ToggleCollapse,
    PageUp, PageDown, Home, End, Invert, SelectNone,
    Accept, Cancel, Resize, ScrollLeft, ScrollRight, Ignore,
}
...
fn visible_rows(actions: &[Action], collapsed: &HashSet<String>) -> Vec<VisibleRow> {
    let mut indexed: Vec<(usize, &str)> = actions.iter().enumerate().map(|(i, a)| (i, a.file.as_str())).collect();
    ...
    for (i, file) in indexed {
        if last != Some(file) {
            rows.push(VisibleRow::FileHeader { file: file.to_string() });
...
fn draw(frame: &mut ratatui::Frame<'_>, actions: &[Action], visible: &[VisibleRow], state: &State) {
    ...
    let (act_w, chg_w, loc_w) = actions.iter().fold((0usize, 0, 0), |(a, c, l), x| {
        ...
        (a.max(x.action_name().chars().count()), c.max(change.chars().count()), l.max(loc.chars().count()))
    });

    let content_width = actions.iter().map(|a| {
        let mut w = 6 + act_w + 2 + chg_w + 2 + loc_w;
        if a.new_ref != a.new_version { w += 3 + a.new_ref.len().min(7) + 1; }
        if a.version_comment.is_some() { w += 1 + a.version_comment.as_ref().unwrap().chars().count() + 1; }
        if a.sha_mismatch { w += 5 + a.expected_sha.len().min(7) + 1; }
        if a.is_branch { w += 7; }
        if a.is_major { w += 6; }
        w
    }).max().unwrap_or(0);
    ...
    let items: Vec<ListItem<'_>> = visible.iter().map(|row| match row {
        VisibleRow::FileHeader { file } => {
            let (sel, total) = actions.iter().zip(state.selected.iter())
                .filter(|(a, _)| a.file == *file)
                .fold((0, 0), |(s, t), (_, x)| (s + usize::from(*x), t + 1));
            let marker = if state.collapsed.contains(file) { "▸" } else { "▾" };
            let label = if sel == total { "all".to_string() } else { format!("{sel}/{total}") };
            let style = |c| Style::default().fg(c).add_modifier(Modifier::BOLD);
            ListItem::new(Line::from(scroll_spans(vec![
                Span::styled(format!("{marker} "), Style::default().fg(Color::Cyan)),
                Span::styled(format!("[{label}] "), style(if sel > 0 { Color::Green } else { Color::DarkGray })),
        ...
        VisibleRow::Update { original_index } => {
            ...
            let marker_s = if sel { Style::default().fg(Color::Green) } else { Style::default().fg(Color::DarkGray) };
            let target_s = if a.sha_mismatch || a.is_branch { Color::Yellow }
                else if a.is_major { Color::Red } else { Color::Green };

            let mut spans = vec![
                Span::raw("  "),
                Span::styled(marker, marker_s),
                Span::raw(" "),
                Span::styled(format!("{:1$}", a.action_name(), act_w), Style::default().add_modifier(Modifier::BOLD)),
                Span::raw("  "),
                Span::styled(format!("{:1$}", format!("{} -> {}", a.current_ref, a.new_version), chg_w), Style::default().fg(target_s)),
                Span::raw("  "),
                Span::styled(format!("{:1$}", format!("{}:{}", a.file, a.line), loc_w), Style::default().fg(Color::DarkGray)),
            ];
            if a.new_ref != a.new_version {
                spans.push(Span::raw("  "));
                spans.push(Span::styled(format!("@{}", &a.new_ref[..a.new_ref.len().min(7)]), Style::default().fg(Color::Blue)));
                spans.push(Span::raw(" "));
            }
            ...
            if a.sha_mismatch { spans.push(Span::styled("sha!", Style::default().fg(Color::Yellow))); spans.push(Span::raw(" ")); }
            if a.is_branch { spans.push(Span::styled("branch", Style::default().fg(Color::Yellow))); spans.push(Span::raw(" ")); }
            if a.is_major { spans.push(Span::styled("major", Style::default().fg(Color::Red))); }
    ...
    let list = List::new(items)
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded)
            .title(format!(" Updates — {selected_count}/{} selected ", actions.len())))
        .highlight_style(Style::default().bg(Color::Rgb(45, 52, 64)).add_modifier(Modifier::BOLD))
        .repeat_highlight_symbol(false);
    frame.render_stateful_widget(list, sections[0], &mut list_state);

    let footer = Paragraph::new(FOOTER)
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded).title(" Keys "))
        .wrap(Wrap { trim: true })
...
fn scroll_spans(spans: Vec<Span<'static>>, scroll: usize) -> Vec<Span<'static>> {
    if scroll == 0 { return spans; }
    let mut remaining = scroll;
    spans.into_iter().flat_map(|span| {
        if remaining == 0 { return vec![span]; }
        let chars: Vec<char> = span.content.chars().collect();
        if remaining >= chars.len() { remaining -= chars.len(); vec![] }
        else { let r = remaining; remaining = 0; vec![Span::styled(chars[r..].iter().collect::<String>(), span.style)] }
    }).collect()
}
...
mod tests {
    ...
    fn a(file: &str, name: &str) -> Action {
        Action::from_scan(
            "actions".into(), name.to_string(), String::new(),
            "v1.0.0".into(), Some("1.0.0".into()),
            file.to_string(), 10, 20, 26,
        )
    }
