...
test "parse workflow uses" {
    ...
    const found = try collectReferences(std.testing.allocator, ".github/workflows/ci.yml", yamlStr);
    defer deinitReferences(std.testing.allocator, found);

    try std.testing.expectEqual(@as(usize, 2), found.len);
    try std.testing.expectEqual(ReferenceKind.workflow_step, found[0].kind);
    try std.testing.expectEqualStrings("build", found[0].scope);
...
test "parse reusable workflow uses" {
    ...
    const found = try collectReferences(std.testing.allocator, ".github/workflows/ci-security.yml", yamlStr);
    defer deinitReferences(std.testing.allocator, found);
...
test "parse composite action uses" {
    ...
    const found = try collectReferences(std.testing.allocator, "action.yml", yamlStr);
    defer deinitReferences(std.testing.allocator, found);
